...
class ReaderView {
     ...
     * @param lang optional language of the article, defaults to en.
     */
    getReadingTime(length, lang = "en") {
        ...
        try {
            let parts = new Intl.RelativeTimeFormat(lang).formatToParts(
                readingTimeMinsSlow,
                "minute"
            );
            if (parts.length === 3) {
                // No need to use part[0] which represents the literal "in".
                const readingTime = parts[1].value; // reading time in minutes
                const minutesLiteral = parts[2].value; // localized singular or plural literal of 'minute'
                let readingTimeString = `${readingTime} ${minutesLiteral}`;
            ...
            console.log("Failed to format reading time:${error.message}");
        }
